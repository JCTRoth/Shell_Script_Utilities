...
- `-y, --yes` : Auto-confirm prompts (requires `--admin-user` and `--admin-key`).
...
- The script DISABLES direct root SSH login and also disables password authentication.
  You MUST ensure an admin user with a working SSH public key is configured before the
  final SSH hardening step or you will be locked out.
- In automatic mode (`--yes`) `--admin-user` and `--admin-key` are required.
