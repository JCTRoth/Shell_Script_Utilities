...
[.lead]
*Features:* Docker CE • k3s (Secured Kubernetes) • SSHGuard • Fail2Ban • Certbot • UFW Firewall • SSH Hardening • Unattended Upgrades
...
|`--ssh-key <key>` |Parameter |SSH public key for authentication setup
|===
...
1. **Port Configuration** - Custom port assignment for services
2. **SSL Setup** - Domain and email configuration for certificates  
3. **SSH Key Management** - Public key installation for secure access
4. **Service Selection** - Choose which components to install
...
sudo ./ubuntu-server-setup.sh --yes
----
Uses default settings for all configurations.
...
[source,bash]
----
sudo ./ubuntu-server-setup.sh --ssh-key "ssh-rsa AAAAB3NzaC1yc2E... user@hostname"
----
...
1. **Network Security** (UFW Firewall + Port Obfuscation)
2. **Access Control** (SSH Hardening + Key-based Auth)
3. **Intrusion Prevention** (Fail2Ban + SSHGuard)
4. **Container Security** (k3s Hardening + PSP)
5. **System Security** (Kernel Hardening + Updates)
